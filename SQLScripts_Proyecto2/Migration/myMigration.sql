...
BEGIN
	BEGIN TRY
		...
		DECLARE @LotXCycleBalance TABLE(ID INT, ServicesBalance FLOAT, SuppliesBalance FLOAT, MachineryBalance FLOAT)
		...
		BEGIN TRANSACTION
			...
			UPDATE AP_LotXCycle SET ServicesBalance = LCB.ServicesBalance,
									SuppliesBalance = LCB.SuppliesBalance,
									MachineryBalance = LCB.MachineryBalance
				FROM AP_LotXCycle LC
				inner join @LotXCycleBalance LCB ON LCB.ID = LC.ID
		COMMIT
	END TRY
	BEGIN CATCH
		IF @@TRANCOUNT = 1
